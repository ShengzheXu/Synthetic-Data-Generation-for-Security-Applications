...
def vali_one(baseline_choice, raw_ip_str, gen_ip_str):
    ...
    print('raw_data/day_1_%s' % raw_ip_str + ',gen_data/%s_%s' % (baseline_choice, gen_ip_str) + ','+ str(KL)) #, ",NMI", NMI)
...
def vali_avg(baseline_choice, raw_ip_str, ten_ips):
    ...
    print('gen_data/%s_%s.csv' % (baseline_choice, raw_ip_str) + ',%s_stat_10csvs' % (baseline_choice) + ','+ str(KL)) #, ",NMI", NMI)
...
# for ips in ten_ips:
#     vali_one(baseline_choice, ips, ips)
