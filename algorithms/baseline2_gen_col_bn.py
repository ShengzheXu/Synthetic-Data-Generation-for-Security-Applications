...
def build_bayesian_dep(all_record, log_byt_col, clf):
    ...
    return pr, bins_name, p_B, p_T_B

def select_bayesian_output(t, b_1, pr, p_B, p_T_B, bins_name):
    # p_B_gv_T_B1 = {}
    maxp = -1
    maxB = -1
    # print(bins_name)
    for interval in bins_name:
        ...
        p_B_gv_T_B1 = p_T_B[t][interval] * p_B[interval] 
        if maxp < p_B_gv_T_B1:
            maxp = p_B_gv_T_B1
            maxB = interval
...
pr, bins_name, p_B, p_T_B = build_bayesian_dep(all_record, np.ravel(byt_log_train), byt_model)
...
#     print(gen_te)
    ...
    # print('t:::', now_t)
    gen_b = select_bayesian_output(now_t, last_b, pr, p_B, p_T_B, bins_name)
